...
Release manifests identify every target, review, version, and historical redirect mapping. Apply is chunked, rotates a checkpoint token, supports resume and idempotent replay, and stores pre/post state for a version-safe rollback. The owner-confirmed greenfield scope keeps all Wix-derived mappings inactive: Wix is a provenance source only and is not a DNS, redirect, hosting, or runtime dependency. The runtime imports only the tracked deterministic redirect config and ignores inactive entries. If a future product decision introduces redirects, activation remains independent and requires a reviewed target version plus exact canonical-origin direct `200` evidence; draft-target mappings additionally require a resolved redirect review.
