...
import { academicYears, activityTypeLabels, mockActivities } from '~/utils/mockData'

definePageMeta({ layout: 'admin' })

const route = useRoute()
const activity = mockActivities.find((item) => item.id === route.params.id) ?? mockActivities[0]
...
const fillForm = (activityData: typeof activity) => {
  form.title = activityData.title
  form.slug = activityData.slug
  form.academicYear = activityData.academicYear
  form.activityType = activityData.activityType
  form.eventDate = activityData.eventDate
  form.location = activityData.location
  form.participantsCount = activityData.participantsCount
  form.resultSummary = activityData.resultSummary
  form.content = activityData.content
  form.coverImageUrl = activityData.coverImageUrl
  form.videoUrl = activityData.videoUrl ?? ''
  form.status = activityData.status
  form.isFeatured = activityData.isFeatured
}

onMounted(async () => {
  submitError.value = ''
  try {
    const activityData = await repository.getActivity(String(route.params.id))
    if (activityData) fillForm(activityData)
  } catch (error) {
    submitError.value = error instanceof Error ? error.message : 'Unable to load activity.'
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    await repository.updateActivity(String(route.params.id), {
      title: form.title,
      slug: form.slug,
    ...
    submitMessage.value = repository.isSupabaseConfigured
      ? 'Activity changes saved to Supabase.'
      : 'Activity changes saved to the local mock store.'
  } catch {
...
<template>
  <div class="grid gap-6">
    <AdminAdminFormSection title="編輯活動" description="調整活動內容、展示圖片與發布狀態。">
      ...
      <form v-else class="grid gap-5" novalidate @submit.prevent="handleSubmit">
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <label class="grid gap-2 text-sm font-semibold text-ink">
            日期
            <input v-model="form.eventDate" type="date" class="focus-ring h-11 rounded-md border px-3" :class="errors.eventDate ? 'border-red-300' : 'border-slate-200'" @input="clearError('eventDate')" />
            <span v-if="errors.eventDate" class="text-xs font-semibold text-red-600">{{ errors.eventDate }}</span>
          </label>
          <label class="grid gap-2 text-sm font-semibold text-ink">
            地點
            <input v-model="form.location" class="focus-ring h-11 rounded-md border px-3" :class="errors.location ? 'border-red-300' : 'border-slate-200'" @input="clearError('location')" />
        ...
        <label class="grid gap-2 text-sm font-semibold text-ink">
          YouTube URL
          <input v-model="form.videoUrl" class="focus-ring h-11 rounded-md border border-slate-200 px-3" />
        </label>
        <label class="grid gap-2 text-sm font-semibold text-ink">
          成果摘要
          <textarea v-model="form.resultSummary" class="focus-ring min-h-24 rounded-md border p-3" :class="errors.resultSummary ? 'border-red-300' : 'border-slate-200'" @input="clearError('resultSummary')" />
          <span v-if="errors.resultSummary" class="text-xs font-semibold text-red-600">{{ errors.resultSummary }}</span>
        </label>
        <label class="grid gap-2 text-sm font-semibold text-ink">
          內文
          <textarea v-model="form.content" class="focus-ring min-h-44 rounded-md border p-3" :class="errors.content ? 'border-red-300' : 'border-slate-200'" @input="clearError('content')" />
