...
export default defineEventHandler(async (event) => {
  ...
  const path = makeContentAssetPath('years', id, extension)
  ...
  const { data, error } = await supabase.from('year_summaries').update({ cover_storage_path: path, cover_alt: altText || null, updated_by: user.id }).eq('id', id).select(yearSelect).single()
  if (error) { await supabase.storage.from(contentAssetsBucket).remove([path]); throwContentDatabaseError(error) }
